...
namespace PesPaise.cs
{
    internal class PesPaise_MET
    {
        ...
        public void Pesquisa_EXEC(string LojaLogada, RadioButton rabAlfabetico, RadioButton rabNumerico, RadioButton rabTodos, RadioButton rabTOP, NumericUpDown nupQtResultados, DataGridView Dgv_Pesquisa, TextBox txtDescri)
        {
            //Cria a conexão com o Banco de Dados e Abre!
            StreamReader StringConexão = new StreamReader(@"..\Conexão\StringConexão.xml", true);
            string LerString = StringConexão.ReadLine();
            ...
            String Select_CMD = String.Format("SELECT " + NúmeroResults + " Codigo_PAI, Descri_PAI FROM TabPaise WHERE 1=1");
            if (!String.IsNullOrEmpty(txtDescri.Text))
                Select_CMD += " AND Descri_PAI LIKE '%' + @1 + '%'";

            if (rabAlfabetico.Checked == true)
                Select_CMD += " ORDER BY Descri_PAI";
            if (rabNumerico.Checked == true)
                Select_CMD += " ORDER BY Codigo_PAI";
            ...
            try
            {
                ...
                while (Dr.Read())
                {
                    if (Dr.HasRows)
                    {
                        string Codigo_MUN = Dr["Codigo_PAI"].ToString().PadLeft(6, '0'); ;
                        string Descri_MUN = Dr["Descri_PAI"].ToString();

                        Dgv_Pesquisa.Rows.Add(Codigo_MUN, Descri_MUN);
                    }
                }
            }
            catch (SqlException Ex)
