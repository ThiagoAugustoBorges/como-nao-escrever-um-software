...
namespace PesMunic.cs
{
    internal class PesMunic_MET
    {
        ...
        public void Pesc_FILTROS(string LojaLogada, RadioButton rabAlfabetico, RadioButton rabNumerico, RadioButton rabTodos, RadioButton rabTOP, NumericUpDown nupQtResultados, DataGridView Dgv_Pesquisa, TextBox txtDescri)
        {
        ...
        public void Pesquisa_EXEC(string LojaLogada, RadioButton rabAlfabetico, RadioButton rabNumerico, RadioButton rabTodos, RadioButton rabTOP, NumericUpDown nupQtResultados, DataGridView Dgv_Pesquisa, TextBox txtDescri, string DescriUF)
        {
            //Cria a conexão com o Banco de Dados e Abre!
            StreamReader StringConexão = new StreamReader(@"..\Conexão\StringConexão.xml", true);
            string LerString = StringConexão.ReadLine();
            ...
            Pesc_FILTROS(LojaLogada, rabAlfabetico, rabNumerico, rabTodos, rabTOP, nupQtResultados, Dgv_Pesquisa, txtDescri);
            #endregion
            ...
            String Select_CMD = String.Format("SELECT " + NúmeroResults + " Codigo_MUN,Descri_MUN,UfFede_MUN,CoFede_MUN FROM TabCidMu WHERE UfFede_MUN = '" + DescriUF + "'");
            if (!String.IsNullOrEmpty(txtDescri.Text))
                Select_CMD += " AND Descri_MUN LIKE '%' + @1 + '%'";



            if (rabAlfabetico.Checked == true)
                Select_CMD += " ORDER BY Descri_MUN";
            if (rabNumerico.Checked == true)
                Select_CMD += " ORDER BY Codigo_MUN";
            ...
            try
            {
                ...
                while (Dr.Read())
                {
                    if (Dr.HasRows)
                    {
                        string Codigo_MUN = Dr["Codigo_MUN"].ToString().PadLeft(6, '0'); ;
                        string Descri_MUN = Dr["Descri_MUN"].ToString();
                        string UfFede_MUN = Dr["UfFede_MUN"].ToString();

                        Dgv_Pesquisa.Rows.Add(Codigo_MUN, Descri_MUN, UfFede_MUN);
                    }
                }
            }
            catch (SqlException Ex)
        ...
        public void GravarFILTROS(CheckBox cheFiltrosPES, RadioButton rabNumerico, RadioButton rabAlfabetica, RadioButton rabTodos, RadioButton rabTOP, NumericUpDown nupQtResultados)
        {
        ...
        //GRAVA O XML DOS FILTROS
        public void CarregaFILTROS(CheckBox cheFiltrosPES, RadioButton rabNumerico, RadioButton rabAlfabetica, RadioButton rabTodos, RadioButton rabTOP, NumericUpDown nupQtResultados)
        {
            string Patch = @"..\Log\Pe_PesMunic.xml";
            try
            {
