...
namespace PesVende.cs
{
    internal class PesVende_MET
    {
        ...
        public string PatchXML = "..\\Log\\Pe_PesVende.xml";
        public string IdentXML = "PesVende";
        ...
        public void Pesc_FILTROS(string LojaLogada, RadioButton rabAlfabetico, RadioButton rabNumerico, RadioButton rabTodos, RadioButton rabTOP, NumericUpDown nupQtResultados, DataGridView Dgv_Pesquisa, ComboBox comStatus, ComboBox comComiss, ComboBox comContrato, TextBox txtCidadeCod, TextBox txtCidadeDesc)
        {
            ...
            if (comComiss.SelectedIndex < 0)
            {
                comComiss.SelectedIndex = 0;
            }

            if (comContrato.SelectedIndex < 0)
            {
                comContrato.SelectedIndex = 0;
            }
        ...
        public void Pesquisa_EXEC(string LojaLogada, RadioButton rabAlfabetico, RadioButton rabNumerico, RadioButton rabTodos, RadioButton rabTOP, NumericUpDown nupQtResultados, DataGridView Dgv_Pesquisa, ComboBox comStatus, TextBox txtCidadeCod, TextBox txtCidadeDesc, TextBox txtDescri, TextBox txtFantasia, TextBox txtCpfCnpj, TextBox txtEmpresa, ComboBox comComiss, ComboBox comContrato)
        {
            ...
            Pesc_FILTROS(LojaLogada, rabAlfabetico, rabNumerico, rabTodos, rabTOP, nupQtResultados, Dgv_Pesquisa, comStatus, comComiss, comContrato, txtCidadeCod, txtCidadeDesc);
            #endregion
            ...
            String Select_CMD = String.Format("SELECT " + NúmeroResults + " Descri_VEN,Fantas_VEN,CpfCnp_VEN,Sequen_VEN,Status_VEN FROM TabVende WHERE 1=1");
            if (!String.IsNullOrEmpty(txtDescri.Text))
                Select_CMD += " AND Descri_VEN LIKE '%' + @1 + '%'";
            if (!String.IsNullOrEmpty(txtFantasia.Text))
                Select_CMD += " AND Fantas_VEN LIKE '%' + @2 + '%'";

            if (!String.IsNullOrEmpty(txtEmpresa.Text))
                Select_CMD += " AND SeqEmp_VEN = " + txtEmpresa.Text;
            if (!String.IsNullOrEmpty(txtCidadeCod.Text))
                Select_CMD += " AND EndCi1_VEN = " + txtCidadeCod.Text;
            if (!String.IsNullOrEmpty(txtCpfCnpj.Text))
                Select_CMD += " AND CpfCnp_VEN LIKE '%' + @3 + '%'";




            if (comStatus.SelectedIndex > 0)
                Select_CMD += " AND Status_VEN = " + comStatus.SelectedIndex;
            if (comComiss.SelectedIndex > 0)
                Select_CMD += " AND Comiss_VEN = " + comComiss.SelectedIndex;
            if (comContrato.SelectedIndex > 0)
                Select_CMD += " AND Contra_VEN = " + comContrato.SelectedIndex;




            if (rabAlfabetico.Checked == true)
                Select_CMD += " ORDER BY Fantas_VEN";
            if (rabNumerico.Checked == true)
                Select_CMD += " ORDER BY Sequen_VEN";
            ...
            ComandoPESQ.Parameters.Add("@3", SqlDbType.VarChar).Value = txtCpfCnpj.Text;
            ...
            try
            {
                ...
                while (Dr.Read())
                {
                    if (Dr.HasRows)
                    {
                        string Sequen_VEN = Dr["Sequen_VEN"].ToString().PadLeft(6, '0');
                        string Descri_VEN = Dr["Descri_VEN"].ToString();
                        string CpfCnp_VEN = Dr["CpfCnp_VEN"].ToString();
                        string Fantas_VEN = Dr["Fantas_VEN"].ToString();
                        string Status_VEN = Dr["Status_VEN"].ToString();

                        #region TRATAMENTO STATUS
                        if (Convert.ToInt32(Status_VEN) == 1)
                        {
                            Status_VEN = "ATIVO";
                        }
                        else if (Convert.ToInt32(Status_VEN) == 2)
                        {
                            Status_VEN = "INATIVO";
                        }
                        else if (Convert.ToInt32(Status_VEN) == 3)
                        {
                            Status_VEN = "EXCLUIDO";
                        }
                        else
                        ...
                        Dgv_Pesquisa.Rows.Add(Sequen_VEN, Descri_VEN, FormatarCpfCnpj(CpfCnp_VEN), Fantas_VEN, Status_VEN);
                    }
                }
            }
            catch (SqlException Ex)
