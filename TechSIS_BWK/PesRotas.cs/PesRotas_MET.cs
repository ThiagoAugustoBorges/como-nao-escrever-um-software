...
namespace PesRotas.cs
{
    internal class PesRotas_MET
    {
        ...
        public string PatchXML = "..\\Log\\Pe_PesRotas.xml";
        public string IdentXML = "PesRotas";
        ...
        public void Pesquisa_EXEC(string LojaLogada, RadioButton rabAlfabetico, RadioButton rabNumerico, RadioButton rabTodos, RadioButton rabTOP, NumericUpDown nupQtResultados, DataGridView Dgv_Pesquisa, TextBox txtDescri)
        {
            ...
            String Select_CMD = String.Format("SELECT " + NúmeroResults + " Sequen_ROT, Descri_ROT, Status_ROT FROM TabRotas WHERE 1=1");
            if (!String.IsNullOrEmpty(txtDescri.Text))
                Select_CMD += " AND Descri_ROT LIKE '%' + @1 + '%'";

            if (rabAlfabetico.Checked == true)
                Select_CMD += " ORDER BY Descri_ROT";
            if (rabNumerico.Checked == true)
                Select_CMD += " ORDER BY Sequen_ROT";
